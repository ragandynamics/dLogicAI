...
app.post("/v1/billing/subscription/change", async (c) => {
  ...
  const targetPlanId = body.plan_id?.trim();
  ...
  // dLogicAI currently supports upgrades only.
  ...
  return c.json({
    success: true,
    message: `Successfully upgraded to ${target.name}.`,
    subscription: {
      id: current.id,
      ...
      monthly_price_cents: target.monthly_price_cents,
      included_requests: target.included_requests,
      included_ai_credit_micros:
        target.included_ai_credit_micros,
      max_projects: target.max_projects,
      max_api_keys: target.max_api_keys,
      max_team_members: target.max_team_members,
      byok_request_fee_micros:
        target.byok_request_fee_micros,
      features_json: target.features_json,
    },
  });
});
/* -------------------------------------------------------------------------- */

...
async function completeCreditReservation(
  c: AppContext,
  tenantId: string,
  requestId: string
) {
  ...
  ).bind(now(), requestId, tenantId).run();
}
...
async function refundCreditReservation(
  ...
  await c.env.DB.batch([
    ...
    ).bind(reservationId.id, reservationId.id, reservationId.id, t, reservationId.id, tenantId),
    c.env.DB.prepare(
      `INSERT INTO credit_ledger (
         id, tenant_id, credit_account_id, entry_type, source, amount,
         balance_after, reference_type, reference_id, description, created_at
...
app.post(
  ...
  async (c) => {
    ...
    if (idempotencyKey && (idempotencyKey.length > 200 || /[\r\n]/.test(idempotencyKey))) {
      return jsonError(c, "INVALID_IDEMPOTENCY_KEY", "Idempotency-Key must be at most 200 characters.", 400);
    }
            ...
            async pull(
              ...
              try {
                ...
                if (done) {
                  ...
                  await completeCreditReservation(c, auth.tenantId, requestId);
...
controller.enqueue(
  encoder.encode(
    `data: ${JSON.stringify({
      ...
      await refundCreditReservation(c, auth.tenantId, requestId).catch(() => undefined);

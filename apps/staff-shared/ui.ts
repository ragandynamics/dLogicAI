import type { Portal } from "./worker";
export function page(portal: Portal) {
...
const labels={features:"Feature availability",overview:"Overview",tenants:"Tenants & balances",usage:"Usage statistics",cases:"Issues & contact enquiries",payments:"Payment history",invoices:"Invoices",ledger:"Credit ledger","billing-events":"Billing events",audit:"Platform audit","tenant-audit":"Tenant audit",system:"System diagnostics",settings:"Configuration",staff:"Staff access"};
const descriptions={
...
const views=["overview","tenants","usage","cases","payments","invoices","ledger","billing-events",...(portal!=="billing"?["audit","tenant-audit","system","settings","features"]:[]),...(portal==="platformadmin"?["staff"]:[])];
...
async function load(){
 ...
 const configuration=current==="settings" || current==='features';
 ...
 try {
  if(configuration){const data=await api(current==='features'?'/api/features':"/api/settings");if(run!==generation)return;current==='features'?renderFeatures(data):renderSettings(data);}
...
function renderFeatures(data){
 $("configuration").innerHTML=data.features.map(f=>'<form class="card" data-feature="'+esc(f.key)+'" data-version="'+f.version+'"><h2>'+esc(f.name)+'</h2><p>'+(!f.implemented?'Not implemented — activation unavailable':f.value.enabled?'Available at platform level':'Unavailable for new starts')+'</p><p class="help">'+(f.parent?'Requires '+esc(f.parent)+'. ':'')+'Saved configurations and records are retained. Calls already sent to a provider may finish.</p><p class="help">Last change: '+esc(f.updated_by||'Default')+' · '+esc(f.updated_at?time(f.updated_at):'No changes')+'</p><fieldset '+(!data.can_manage||!f.implemented?'disabled':'')+'><label><span>Available</span><input type="checkbox" name="enabled" '+(f.value.enabled?'checked':'')+'></label><label>When unavailable<select name="stop"><option value="immediate" '+(f.value.stop==='immediate'?'selected':'')+'>Stop on next request</option>'+(f.key!=='connector.actions'?'<option value="finish_existing" '+(f.value.stop==='finish_existing'?'selected':'')+'>Allow existing conversations to continue</option>':'')+'</select></label><label>Reason<input name="reason" maxlength="300" value="'+esc(f.value.reason)+'"></label><button class="primary">Save availability</button></fieldset><p role="status"></p></form>').join('');
 $("configuration").querySelectorAll('form').forEach(form=>form.onsubmit=async e=>{
  e.preventDefault();const fields=form.elements,button=form.querySelector('button'),notice=form.querySelector('[role="status"]');button.disabled=true;
  try{const result=await api('/api/settings/feature.'+encodeURIComponent(form.dataset.feature),{method:'POST',body:JSON.stringify({version:Number(form.dataset.version),value:{enabled:fields.enabled.checked,stop:fields.stop.value,reason:fields.reason.value}})});form.dataset.version=String(result.version);notice.textContent='Saved. Availability is enforced on subsequent requests.';}catch(error){notice.textContent=error.message;}finally{button.disabled=false;}
 });
 if(data.can_manage){
  const form=document.createElement('form');form.className='card';form.innerHTML='<h2>Tenant feature access</h2><p class="help">Allow preserves existing plan limits and platform restrictions. Deny blocks subsequent use, including existing conversations.</p><label>Tenant ID<input name="tenant" required maxlength="200"></label><label>Feature<select name="feature">'+data.features.filter(f=>f.implemented).map(f=>'<option value="'+esc(f.key)+'">'+esc(f.name)+'</option>').join('')+'</select></label><button type="button" data-load>Load tenant access</button><fieldset disabled><label>Allow use<input name="enabled" type="checkbox"></label><button class="primary">Save tenant access</button></fieldset><p role="status"></p>';
  let loaded=null;const status=form.querySelector('[role="status"]'),fields=form.elements;
  const reset=()=>{loaded=null;form.querySelector('fieldset').disabled=true;};fields.tenant.oninput=reset;fields.feature.onchange=reset;
  form.querySelector('[data-load]').onclick=async()=>{reset();try{const path='/api/features/'+encodeURIComponent(fields.feature.value)+'/tenants/'+encodeURIComponent(fields.tenant.value.trim()),r=await api(path);loaded={path,version:r.version};fields.enabled.checked=r.enabled;form.querySelector('fieldset').disabled=false;status.textContent='Access loaded.';}catch(error){status.textContent=error.message;}};
  form.onsubmit=async e=>{e.preventDefault();if(!loaded)return;try{const result=await api(loaded.path,{method:'POST',body:JSON.stringify({enabled:fields.enabled.checked,version:loaded.version})});loaded.version=result.version;status.textContent='Tenant access saved.';}catch(error){status.textContent=error.message;}};
